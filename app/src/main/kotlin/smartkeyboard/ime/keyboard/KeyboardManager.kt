...
package smartkeyboard.ime.keyboard
...
import smartkeyboard.clipboardManager
import smartkeyboard.editorInstance
import smartkeyboard.extensionManager
...
import smartkeyboard.ime.editor.ImeOptions
import smartkeyboard.ime.editor.InputAttributes
import smartkeyboard.ime.input.InputEventDispatcher
import smartkeyboard.ime.input.InputKeyEventReceiver
import smartkeyboard.ime.input.InputShiftState
import smartkeyboard.ime.nlp.ClipboardSuggestionCandidate
import smartkeyboard.ime.nlp.PunctuationRule
import smartkeyboard.ime.nlp.SuggestionCandidate
import smartkeyboard.ime.onehanded.OneHandedMode
import smartkeyboard.ime.popup.PopupMappingComponent
import smartkeyboard.ime.text.composing.Composer
import smartkeyboard.ime.text.gestures.SwipeAction
import smartkeyboard.ime.text.key.KeyCode
import smartkeyboard.ime.text.key.KeyType
import smartkeyboard.ime.text.key.UtilityKeyAction
import smartkeyboard.ime.text.keyboard.TextKeyData
import smartkeyboard.ime.text.keyboard.TextKeyboardCache
import smartkeyboard.lib.android.showLongToast
...
import smartkeyboard.lib.kotlin.collectIn
import smartkeyboard.lib.kotlin.collectLatestIn
import smartkeyboard.lib.kotlin.titlecase
import smartkeyboard.lib.kotlin.uppercase
import smartkeyboard.lib.util.InputMethodUtils
import smartkeyboard.nlpManager
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
    ...
    private val _activeSmartbarEvaluator = MutableStateFlow<ComputingEvaluator>(
        DefaultComputingEvaluator
    )
    val activeSmartbarEvaluator get() = _activeSmartbarEvaluator.asStateFlow()
    private val _lastCharactersEvaluator = MutableStateFlow<ComputingEvaluator>(
        DefaultComputingEvaluator
    )
    ...
    fun reevaluateInputShiftState() {
        if (activeState.inputShiftState != InputShiftState.CAPS_LOCK && !inputEventDispatcher.isPressed(
                KeyCode.SHIFT)) {
    ...
    fun handleArrow(code: Int, count: Int = 1) = editorInstance.apply {
        val isShiftPressed = activeState.isManualSelectionMode || inputEventDispatcher.isPressed(
            KeyCode.SHIFT)
    ...
    override fun onInputKeyUp(data: KeyData) = activeState.batchEdit {
        when (data.code) {
            ...
            KeyCode.VIEW_NUMERIC_ADVANCED -> activeState.keyboardMode =
                KeyboardMode.NUMERIC_ADVANCED
